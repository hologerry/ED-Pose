...
def add_box_to_img(img, boxes, colorlist, brands=None):
    ...
    for _i, (box, color) in enumerate(zip(boxes, colorlist)):
        ...
        if brands is not None:
            ...
            fontScale = 0.5
            thickness = 1
            img = cv2.putText(img.copy(), str(brand), org, font, fontScale, color, thickness, cv2.LINE_AA)
    return img
...
def plot_raw_img(img, boxes, labels):
    ...
    for box, label in zip(boxes.tolist(), labels.tolist()):
        ...
        # import ipdb; ipdb.set_trace()
        img = cv2.rectangle(
            img.copy(), (int(x - w / 2), int(y - h / 2)), (int(x + w / 2), int(y + h / 2)), _color_getter(label), 2
        )
        # add text
        org = (int(x - w / 2), int(y + h / 2))
        font = cv2.FONT_HERSHEY_SIMPLEX
        fontScale = 1
        thickness = 1
        img = cv2.putText(img.copy(), str(label), org, font, fontScale, _color_getter(label), thickness, cv2.LINE_AA)
