...
class COCOVisualizer:
    ...
    def visualize(self, img, tgt, caption=None, dpi=180, savedir="vis"):
        ...
        if caption is None:
            savename = "{}/{}-{}.png".format(
                savedir, int(tgt["image_id"]), str(datetime.datetime.now()).replace(" ", "-")
            )
        else:
            savename = "{}/{}-{}-{}.png".format(
                savedir, caption, int(tgt["image_id"]), str(datetime.datetime.now()).replace(" ", "-")
            )
